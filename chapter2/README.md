...
　多くの場合はメソッド呼び出しによって機能が実現している。このあたりの設計は一通り定石と化している。今回の__float128型のように，言語を拡張する数値型はRubyでは実数を仮定している。例えば実数かどうかを評価するメソッド#real?では、Numeric型より継承した場合、デフォルトでtrueが返却されるようになっている。
