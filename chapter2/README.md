...
　数値計算でプリミティブ型というと，整数クラス・実数クラス・複素数クラスとあり，実数は整数の拡張，複素数は実数の拡張という位置づけになる．この拡張は，計算結果にも影響を及ぼしている．  
...
　これは整数と実数がオペランドにあった場合，計算結果は実数に，整数および実数と複素数がオペランドにあった場合，計算結果は複素数になる，という数値計算の仕組みである．実装としてはプライオリティ{優先順位}という．  
